...
다행히 Flutter에는 위젯이 해당 하위 항목(즉, 해당 하위 항목뿐만 아니라 그 아래에 있는 모든 위젯)에 데이터와 서비스를 제공하는 메커니즘이 있습니다. 모든 것이 Widget™인 Flutter에서 기대할 수 있듯이 이러한 메커니즘은 InheritedWidget, InheritedNotifier, InheritedModel 등과 같은 특별한 종류의 위젯일 뿐입니다. 우리가 하려는 일에 비해 약간 로우 레벨이기 때문에 여기서는 다루지 않겠습니다.
