...
모든 Dart 코드는 기본 메인 아이솔레이트에서 시작하여 명시적으로 생성하는 후속 아이솔레이트로 확장될 수 있는 아이솔레이트에서 실행됩니다. 새로운 아이솔레이트를 생성하면 고유한 메모리와 이벤트 루프를 가지게 됩니다. 이벤트 루프는 Dart에서 비동기 및 동시성 프로그래밍을 가능하게 합니다.
