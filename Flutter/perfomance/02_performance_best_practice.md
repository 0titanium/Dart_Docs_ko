...
Flutter 앱에서 장면을 가장 효율적으로 렌더링하려면 어떻게 설계해야 할까요? 특히, 프레임워크가 생성하는 페인팅 코드가 가능한 한 효율적이도록 하는 방법이 중요합니다. 일부 렌더링 및 레이아웃 작업은 느릴 수 있지만, 항상 피할 수는 없습니다. 이러한 작업은 아래의 가이드를 참고하여 신중하게 사용해야 합니다.

## 비용이 많이 드는 작업 최소화 Minimize expensive operations

일부 작업은 다른 작업보다 더 많은 리소스를 소비하므로, 필요할 때만 사용하는 것이 좋습니다. 앱의 UI를 어떻게 설계하고 구현하느냐에 따라 실행 효율성이 크게 달라질 수 있습니다.
